...
@synthesize itemBadge = _itemBadge;
...
title:(NSString *)ctitle;
{
    if (self = [super init])
    {
        ...
        [_itemLabel setFont:[UIFont systemFontOfSize:14]];
        _itemLabel.text = ctitle;
...
- (void)layoutSubviews
{
    ...
    _itemLabel.frame = CGRectMake( - self.bounds.size.width/2, self.bounds.size.height + 10 - height/2, 2*width, height);
    [_itemLabel setTextAlignment:NSTextAlignmentCenter];
    [_itemLabel setFont:[UIFont fontWithName:@"HelveticaNeue" size:12]];
    [_itemLabel setTextColor:[UIColor darkGrayColor]];
}

- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    self.highlighted = YES;
    if ([_delegate respondsToSelector:@selector(RCMenuItemTouchesBegan:)])
    {
        [_delegate RCMenuItemTouchesBegan:self];
    }
